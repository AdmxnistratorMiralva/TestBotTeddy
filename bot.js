...
const strigalog = guild.channels.find(c=> c.id ==="") // MSJ GDCK KANL ID 
...
guild.owner.send(`Striga Güvenliği | ** <@${yetkili.id}> İsimili Yetkili <@${user.id}>** Adlı Kişiyi Banladı Ve Yetkilerini Aldım.`)
},2000)
})
...
client.on("guildMemberRemove", async function(member) {
  ...
  setTimeout(async () => { // S T G 
    ...
    const strigalog = guild.channels.find(c => c.id === ""); // log kanal id
...
client.on("roleDelete", async (role) => { 
  ...
  let position = role.position;
     const strigalog = role.guild.channels.find(c=> c.id ==="") // MESAJ ATICAĞI KANAL ID'Sİ
const striga = new Discord.RichEmbed()
.setColor('RANDOM')
.setDescription(`**<@${deleter.id}> ${role.name}** Adlı Rol Silindi Ben Terkar Oluşturdum Ve Tüm Yetkileri Alınıp <@&> Rolü Verildi.`)
strigalog.send(striga)
  role.guild.owner.send(`**<@${deleter.id}> Yetkili  ${role.name}** Adlı Rol Silindi Ben Terkar Oluşturdum Ve Tüm Yetkileri Alınıp <@&> Rolü Verildi.`)
    ...
    setTimeout(function(){
                         ...
                         role.client.channels.get(``).send(); // YİNE KANAL IDSİ LAN BEN BU SİSTEMİ NASIL YAPTIM AQ
                         }, 1500);
 ...
 client.on('roleCreate', async (role) => {
  ...
  if(yetkili.id == "") return; // VAY BENİM DEVREM NASILSIN ENİŞTE ID  
    ...
    setTimeout(function(){
                         ...
                         role.client.channels.get(``).send(strigalog); // MESAJIN GİDİCEĞİ KANALIN IDSI
                         }, 1500); //
